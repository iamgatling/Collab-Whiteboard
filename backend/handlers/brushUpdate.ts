...
export default (io: Server, socket: Socket, roomManager: RoomManager, supabase: SupabaseClient, logToSupabase: typeof logToSupabase, state: State) => ({ color, width }: { color: string, width: number }) => {
  try {
    ...
    logToSupabase({
      ...
      roomId: state.currentRoomId,
      userId: state.currentUserId,
      message: `Error in ${eventNames.BRUSH_UPDATE} event: ${(err as Error).message}`,
      data: { error: { message: (err as Error).message, stack: (err as Error).stack }, function: 'brushUpdateHandler' }
    });
  }
};
