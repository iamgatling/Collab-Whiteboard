...
export default (io: Server, socket: Socket, roomManager: RoomManager, supabase: SupabaseClient, logToSupabase: typeof logToSupabase, state: State) => async ({ canvasData, textData }: { canvasData: any, textData: string }) => {
  try {
    ...
    logToSupabase({ type: 'ERROR', roomId: state.currentRoomId!, userId: socket.id, message: `Error in periodic save for room ${state.currentRoomId}. Error: ${(err as Error).message}`, data: { error: { message: (err as Error).message, stack: (err as Error).stack }, function: 'periodicSaveHandler' } });
