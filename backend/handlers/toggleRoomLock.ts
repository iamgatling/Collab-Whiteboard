...
export default (io: Server, socket: Socket, roomManager: RoomManager, supabase: SupabaseClient, logToSupabase: typeof logToSupabase, state: State) => (data: any, callback: (response: { success: boolean, error?: string, isLocked?: boolean }) => void) => {
  ...
  if (result.success) {
    ...
    try {
      ...
      logToSupabase({ type: 'ERROR', roomId: state.currentRoomId, userId: state.currentUserId, message: 'Exception when logging room_lock_toggled action.', data: { error: { message: (error as Error).message, stack: (error as Error).stack } } });
    }
    try {
      const lockAction = {
        room_id: state.currentRoomId,
      ...
      console.error('Exception logging room_lock_toggled action:', error);
      logToSupabase({ type: 'EXCEPTION_LOGGING_ERROR', roomId: state.currentRoomId, userId: state.currentUserId, message: 'Exception when logging room_lock_toggled to user_actions_log.', data: { error: { message: (error as Error).message, stack: (error as Error).stack } } });
    }
