...
export default (io: Server, socket: Socket, roomManager: RoomManager, supabase: SupabaseClient, logToSupabase: typeof logToSupabase, state: State) => () => {
  ...
  roomsUserWasIn.forEach(async (rId) => {
    ...
    if (removed) {
      ...
      try {
        ...
        logToSupabase({ type: 'EXCEPTION_LOGGING_ERROR', roomId: rId, userId: state.currentUserId, message: 'Exception when logging user_left to user_actions_log.', data: { error: { message: (error as Error).message, stack: (error as Error).stack } } });
      }
