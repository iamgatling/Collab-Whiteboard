...
export default (
  ...
  () => {
    logToSupabase({
      type: "USER_DISCONNECTED_SOCKET",
      userId: state.currentUserId,
      roomId: state.currentRoomId,
      message: `User socket ${state.currentUserId} disconnected.`,
    });
    ...
    roomsUserWasIn.forEach(async (rId) => {
      ...
      if (removed) {
        try {
          ...
          const { error: leftLogError } = await supabase
            .from("user_actions_log")
            .insert([leftAction])
            .select();
          if (leftLogError) {
            console.error(
              "[RAW SUPABASE ERROR user_left_action]",
              leftLogError
            );
            console.error(
              `Supabase error logging user_left action for room ${rId}:`,
              leftLogError
            );
            logToSupabase({
              type: "LOGGING_ERROR",
              roomId: rId,
              userId: state.currentUserId,
              message: "Failed to log user_left to user_actions_log.",
              data: {
                error: {
                  message: leftLogError.message,
                  code: leftLogError.code,
                  details: leftLogError.details,
                  hint: leftLogError.hint,
                  stack: leftLogError.stack,
                },
              },
            });
          }
          ...
          console.error(
            `Exception logging user_left action for room ${rId}:`,
            error
          );
          logToSupabase({
            ...
            roomId: rId,
            userId: state.currentUserId,
            message: "Exception when logging user_left to user_actions_log.",
            data: {
              error: {
        ...
        io.to(rId).emit(eventNames.USER_LEFT, {
          userId: state.currentUserId,
          userName: currentUserNameForRoom,
        });
        if (remainingParticipants) {
          ...
          if (wasHost && remainingParticipants.length > 0) {
            ...
            if (newHost) {
              io.to(rId).emit(eventNames.HOST_CHANGED, {
                roomId: rId,
                newHostId: newHost,
                oldHostId: state.currentUserId,
              });
              io.to(rId).emit(
