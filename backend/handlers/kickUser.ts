...
export default (
  ...
  (
    data: { userIdToKick: string },
    callback: (response: { success: boolean; error?: string }) => void
  ) => {
    const { userIdToKick } = data;
    if (!state.currentRoomId || !userIdToKick) {
      if (callback)
        callback({ success: false, error: "Missing room or user ID to kick." });
      return;
    }
    ...
    const userToKickDetails = participants?.find(
      (p) => p.userId === userIdToKick
    );
    if (!userToKickDetails) {
      if (callback)
        callback({ success: false, error: "User not found in this room." });
      return;
    }

    const wasKickedUserHost =
      roomManager.getRoomHost(state.currentRoomId) === userIdToKick;
    ...
    if (removed) {
      ...
      if (kickedSocket) {
        kickedSocket.emit(eventNames.YOU_WERE_KICKED, {
          roomId: state.currentRoomId,
          reason: "Kicked by host.",
        });
        kickedSocket.leave(state.currentRoomId);
      }
      io.to(state.currentRoomId).emit(eventNames.USER_LEFT, {
        userId: userIdToKick,
        userName: userToKickDetails.userName,
        reason: "kicked",
      });
      const remainingParticipants = roomManager.getRoomParticipants(
        state.currentRoomId
      );
      io.to(state.currentRoomId).emit(
        eventNames.USERS_COUNT,
      ...
      try {
        const kickAction = {
          ...
          details: {
            ...
            kickedUserName: userToKickDetails.userName,
            kickedBy: state.currentUserName,
          },
        };
        async function insertkickAction(kickAction: any) {
          const { error: kickLogError } = await supabase
        ...
        logToSupabase({
          ...
          message: "Exception when logging user_kicked action.",
          data: {
            error: {
      ...
      if (
        ...
        if (newHost) {
          io.to(state.currentRoomId).emit(eventNames.HOST_CHANGED, {
            roomId: state.currentRoomId,
            newHostId: newHost,
            oldHostId: userIdToKick,
          });
          io.to(state.currentRoomId).emit(
            eventNames.ROOM_PARTICIPANTS,
            remainingParticipants
          );
        }
      }
      ...
      try {
        ...
        logToSupabase({
          ...
          roomId: state.currentRoomId,
          userId: state.currentUserId,
          message: "Exception when logging user_kicked to user_actions_log.",
          data: {
            error: {
              message: (error as Error).message,
              stack: (error as Error).stack,
